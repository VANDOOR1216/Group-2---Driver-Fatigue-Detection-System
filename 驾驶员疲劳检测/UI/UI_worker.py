...
class RealtimeDetectionWorker(QThread):
    ...
    # 信号定义
    frame_ready = pyqtSignal(np.ndarray)  # 视频帧信号
    ...
    alarm_triggered = pyqtSignal(bool)    # 报警触发信号
    ...
    def _process_frame(self, frame):
        ...
        if main_face is not None:
            ...
            if face_bbox:
                ...
                if landmarks is not None:
                    ...
                    # 绘制状态文本
                    status_text = f"状态: {fatigue_state.value}"
                    ...
                    blink_count = stats.get('blink_count', 0)
                    yawn_count = stats.get('yawn_count', 0)
                    cv2.putText(frame, f"眨眼: {blink_count}", (10, 100),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
                    cv2.putText(frame, f"哈欠: {yawn_count}", (10, 120),
                              cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1)
        ...
        # 绘制录制状态
        if self.recording:
            cv2.putText(frame, "录制中...", (width - 100, 60),
        ...
        try:
            ...
            if self.detection_results:
                ...
                data['statistics'] = {
                    ...
                    'total_fatigue_frames': fatigue_states.count('轻度疲劳') + fatigue_states.count('重度疲劳'),
